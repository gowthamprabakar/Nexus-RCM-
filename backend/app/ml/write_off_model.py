...
class WriteOffModel:
    ...
    def predict(self, features: dict) -> dict:
        ...
        if not self._is_fitted:
            ...
            prob = min(1.0, max(0, aging / 365))
            return {"write_off_probability": round(prob, 4),
                    "days_to_write_off_estimate": max(0, 180 - aging),
    ...
    async def _extract_training_data(self, db: AsyncSession) -> tuple:
        query = text("""
            SELECT
                CURRENT_DATE - c.date_of_service AS aging_days,
                CASE pm.payer_group
    ...
    async def _extract_claim_features(self, db: AsyncSession, claim_id: str) -> Optional[dict]:
        query = text("""
            SELECT
                CURRENT_DATE - c.date_of_service,
                pm.payer_group, c.total_charges, pm.denial_rate, pm.adtp_days,
                c.status, c.submission_date, c.date_of_service,
        ...
        try:
            ...
            return {
                "aging_days": int(row[0] or 0),
                "payer_type_encoded": pg_map.get(row[1], 7),
                "denial_count": int(row[8] or 0),
                "appeal_count": int(row[9] or 0),
                "total_charges": float(row[2] or 0),
                "payment_ratio": float(row[10] or 0) / max(float(row[2] or 1), 1),
                "payer_denial_rate": float(row[3] or 0),
                "has_been_resubmitted": 1 if row[5] == "SUBMITTED" else 0,
                "claim_line_count": 1,
                "payer_adtp_days": float(row[4] or 30),
            }
        except Exception as exc:
