...
export function AIInsightsPanel({ insights, onInsightClick }) {
 ...
 return colors[impact] || 'border-th-border-strong';
 };
 ...
 return badges[category] || { label: 'General', color: 'bg-th-surface-overlay text-th-primary' };
 };

 return (
 <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-slate-900 dark:to-slate-800 border border-blue-100 p-6 rounded-xl shadow-sm">
 <div className="flex items-center gap-3 mb-6">
 <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center">
 <span className="material-symbols-outlined text-th-heading text-2xl">psychology</span>
 </div>
 <div>
 <h3 className="text-lg font-bold text-th-heading ">AI-Powered Insights</h3>
 <p className="text-sm text-th-muted ">Actionable recommendations to optimize collections</p>
 </div>
 </div>
 ...
 <div
 key={insight.id}
 className={`border-l-4 ${getImpactColor(insight.impact)} bg-white p-4 rounded-lg cursor-pointer hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5`}
 ...
 <h4 className="font-bold text-th-heading text-sm">{insight.title}</h4>
 <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${badge.color}`}>
 {badge.label}
 </span>
 </div>
 <p className="text-sm text-th-muted mb-2">{insight.description}</p>
 ...
 <div className="text-xs text-th-muted mb-1">Confidence</div>
 <div className="text-lg font-black text-th-heading ">{insight.confidence}%</div>
 </div>
 </div>
 <div className="flex items-center justify-between mt-3 pt-3 border-t border-th-border-subtle ">
 <div className="text-sm">
 <span className="text-th-muted ">Potential Recovery: </span>
