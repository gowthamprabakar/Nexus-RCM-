...
export function BatchReadinessMeter({ percentage }) {
 ...
 <span className="text-xs font-bold text-th-muted uppercase tracking-wider">Batch Readiness</span>
 <span className={`text-lg font-black ${percentage >= 98 ? 'text-emerald-600' : 'text-th-heading '}`}>
 {percentage}%
 </span>
 </div>
 ...
 {percentage < 98 && (
 <p className="text-[10px] text-th-secondary mt-1 text-right">Target: 98% to submit</p>
 )}
 </div>
 );
}
