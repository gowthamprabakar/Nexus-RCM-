...
function PaymentDelayIndicator({ claimId, compact = false }) {
  ...
  useEffect(() => {
    ...
    api.predictions.getPaymentDelay(claimId)
      .then((res) => {
        if (res && res.estimated_days !== undefined) {
          setData(res);
        } else {
          setError(true);
        }
  ...
  const days = data.estimated_days;
  ...
  if (compact) {
    return (
      <span className={`inline-flex items-center gap-1 ${colorClasses.text}`}>
        ...
        <span className="text-[10px] font-bold tabular-nums">Est. {days}d</span>
      </span>
    );
  }
  ...
  return (
    <span
      ...
      title={`Estimated payment delay: ${days} days`}
    >
...
export default PaymentDelayIndicator;
