...
# === RCM SIMULATION ENDPOINTS ===

@simulation_bp.route('/rcm/run', methods=['POST'])
def run_rcm_simulation():
    """Run RCM multi-agent simulation"""
    import asyncio
    from ..services.rcm_simulation import get_simulation_engine
    ...
    try:
        ...
        result = loop.run_until_complete(engine.run_simulation(scenario))
...
@simulation_bp.route('/rcm/validate', methods=['POST'])
def validate_suggestion():
    """Validate a suggestion before showing to user"""
    import asyncio
    from ..services.rcm_validator import get_validator
    ...
    loop = asyncio.new_event_loop()
    try:
        validator = loop.run_until_complete(get_validator())
        result = loop.run_until_complete(validator.validate(suggestion, validation_type))
        return jsonify(result.to_dict())
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
...
@simulation_bp.route('/rcm/scheduler/status', methods=['GET'])
def get_scheduler_status():
    """Get scheduler state"""
    from ..services.rcm_scheduler import get_scheduler
    scheduler = get_scheduler()
    return jsonify(scheduler.get_latest_results())

@simulation_bp.route('/rcm/latest', methods=['GET'])
def get_latest_results():
    """Get all cached simulation results"""
    from ..services.rcm_scheduler import get_scheduler
    scheduler = get_scheduler()
    return jsonify(scheduler.state.latest_results)

@simulation_bp.route('/rcm/latest/<scenario_id>', methods=['GET'])
def get_scenario_result(scenario_id):
    """Get cached result for one scenario"""
    from ..services.rcm_scheduler import get_scheduler
    scheduler = get_scheduler()
    result = scheduler.get_scenario_result(scenario_id)
    if result:
        return jsonify(result)
    return jsonify({"error": "Not found"}), 404

@simulation_bp.route('/rcm/run-now/<scenario_id>', methods=['POST'])
def run_scenario_now(scenario_id):
    """Run a specific scenario immediately"""
    import asyncio
    from ..services.rcm_scheduler import get_scheduler
    ...
    loop = asyncio.new_event_loop()
    try:
        result = loop.run_until_complete(scheduler.run_scenario_now(scenario_id))
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
...
@simulation_bp.route('/rcm/agents', methods=['GET'])
def get_rcm_agents():
    """Get all payer digital twin agents"""
    import asyncio
    from ..services.rcm_agent_generator import get_agent_generator

    loop = asyncio.new_event_loop()
    try:
        gen = loop.run_until_complete(get_agent_generator())
        agents = gen.get_all_agents()
...
@simulation_bp.route('/rcm/interview/<agent_id>', methods=['POST'])
def rcm_interview_agent(agent_id):
    """Interview a specific RCM payer agent"""
    import asyncio
    from ..services.rcm_agent_generator import get_agent_generator
